# Import Agent, MCPClient and streamablehttp_client from the strands library
...
def main():
    ...
    # Create a streamable http MCPClient connecting to "http://localhost:8080/mcp"

    def create_streamable_http_transport():
        return streamablehttp_client("http://localhost:8081/mcp")

    streamable_http_mcp_client = MCPClient(create_streamable_http_transport)
    
    try:
        # Use the MCP client in a context manager (with statement)
        with streamable_http_mcp_client:
            # Get available tools from MCP server using list_tools_sync()
