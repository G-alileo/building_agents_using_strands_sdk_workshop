...
Available agents:
- Rules Agent (http://127.0.0.1:8082) - For D&D mechanics and rules
- Character Agent (http://127.0.0.1:8083) - For character creation and management
...
A2A_AGENT_URLS = [
    "http://127.0.0.1:8082",  # Rules Agent
    "http://127.0.0.1:8083",  # Character Agent
]
...
async def ask_agent(request: QuestionRequest):
    ...
    try:
        with mcp_client:
            ...
            # Strip markdown code fences if agent wraps response in ```json ... ```
            ...
            message_history.extend(agent.messages)
